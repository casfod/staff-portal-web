...
import { FileText, Tag, DollarSign, Building, User, CheckCircle, XCircle } from 'lucide-react';
...
export const PaymentVoucherDetails = ({ voucher }: PaymentVoucherDetailsProps) => {
  const { voucherId } = useParams();

  const getPosition = (user: Partial<IUser> | undefined): string => {
    if (!user) return 'N/A';
      ...
      fields: [
        {
          ...
          content: (
            <div>
              <div className="font-medium">{getUserFullName(voucher.createdBy)}</div>
              <div className="text-sm text-gray-600">{getPosition(voucher.createdBy)}</div>
            </div>
          ),
        ...
        },
        {
          ...
          content: (
            <div>
              <div className="font-medium">{getUserFullName(voucher.reviewedBy)}</div>
              <div className="text-sm text-gray-600">{getPosition(voucher.reviewedBy)}</div>
            </div>
          ),
          icon: <User className="w-4 h-4" />,
        },
        {
          ...
          content: (
            <div>
              <div className="font-medium">{getUserFullName(voucher.approvedBy)}</div>
              <div className="text-sm text-gray-600">{getPosition(voucher.approvedBy)}</div>
            </div>
          ),
        ...
        <div className="rounded-lg p-4 bg-gray-50 border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-3 bg-white rounded border border-gray-100">
              <div className="text-sm font-semibold text-gray-600 mb-1">Gross Amount</div>
              <div className="text-lg font-bold text-gray-800">
                {moneyFormat(voucher.grossAmount, 'NGN')}
              </div>
            </div>
            <div className="text-center p-3 bg-white rounded border border-gray-100">
              <div className="text-sm font-semibold text-red-600 mb-1">Total Deductions</div>
              <div className="text-lg font-bold text-red-800">
                {moneyFormat(totalDeductions, 'NGN')}
              </div>
            </div>
            <div className="text-center p-3 bg-white rounded border border-gray-100">
              <div className="text-sm font-semibold text-gray-600 mb-1">Net Amount</div>
              <div className="text-lg font-bold text-gray-800">
