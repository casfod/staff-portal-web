...
import { IExpenseClaim, IProject, IAccountCode, IExpenseItem } from '../../interfaces';
import { expenses } from '../../assets/expenses';

type ExpenseClaimFormData = Partial<
  Omit<IExpenseClaim, 'reviewedBy' | 'approvedBy' | 'project'>
> & {
  accountCode: string;
  ...
  expenseClaim: { from: string; to: string };
};

interface ExpenseClaimFormProps {
  mode: 'create' | 'edit';
  initialData?: IExpenseClaim;
}
...
const ExpenseClaimForm: React.FC<ExpenseClaimFormProps> = ({ mode, initialData }) => {
  const [formData, setFormData] = useState<ExpenseClaimFormData>(() => {
    if (mode === 'edit' && initialData) {
      return {
        expenseReason: initialData.expenseReason || '',
        expenseClaim: initialData.expenseClaim || { from: '', to: '' },
  ...
  const { saveExpenseClaim, isPending: isSaving } = useSaveExpenseClaim();
  ...
  const totalBudget = useMemo(() => {
    return expenseItems.reduce((sum, item) => sum + (item.total || 0), 0);
  }, [expenseItems]);
  ...
  const handleFormChange = useCallback(
    (field: keyof ExpenseClaimFormData, value: string | number | null) => {
  ...
  const handleNestedChange = useCallback(
    (
      parentField: keyof ExpenseClaimFormData,
      field: string,
  ...
  const buildSubmitData = useCallback((): Partial<IExpenseClaim> => {
    ...
    return {
      ...
      expenseClaim: formData.expenseClaim,
  ...
  const handleSave = useCallback(
    ...
    [buildSubmitData, mode, saveExpenseClaim, updateExpenseClaim]
  );
  ...
  const handleSend = useCallback(
    ...
    [buildSubmitData, sendExpenseClaim]
  );
    ...
    <form className="space-y-6">
      {mode === 'edit' && initialData && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div>
            <span className="font-bold uppercase">EC Number:</span>
            <span className="ml-2">{initialData.ecNumber}</span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormRow label="Departure (From) *">
          <Input
            ...
            onChange={e => handleNestedChange('expenseClaim', 'from', e.target.value)}
            disabled={isPending}
          />
        </FormRow>
        ...
        <FormRow label="Destination (To) *">
          <Input
            ...
            onChange={e => handleNestedChange('expenseClaim', 'to', e.target.value)}
            disabled={isPending}
          />
        </FormRow>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto p-4 border rounded-lg">
          {expenseItems.map((item, index) => renderItem(item, index))}
        </div>

        <div className="flex justify-between items-center">
          <p className="text-xs text-gray-500">
            {expenseItems.length} expense{expenseItems.length > 1 ? 's' : ''} added
          </p>
