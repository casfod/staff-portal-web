...
const FormEditExpenseClaim: React.FC<FormEditExpenseClaimProps> = ({
  expenseClaim,
}) => {
  ...
  const [formData, setFormData] = useState<ExpenseClaimType>({
    ...
    dayOfDeparture: expenseClaim.dayOfDeparture,
    dayOfReturn: expenseClaim.dayOfReturn,
    expenses: [...expenseClaim.expenses],
    expenseChargedTo: expenseClaim.expenseChargedTo,
    accountCode: expenseClaim.accountCode,
...
p-3 md:p-6 mb-3 rounded-lg shadow-md"
      ...
      {expenseClaim.reviewedBy ? (
        <div className=" ">
          ...
          {expenseClaim?.comments && (
            <div className="mb-2">
              <span className="font-bold mr-1  uppercase">Comments :</span>

              {expenseClaim?.comments?.map((comment) => (
      ...
      {expenseClaim.status !== "rejected" && formData.reviewedBy && (
        <FileUpload
...
export default FormEditExpenseClaim;
