// ExpenseClaim.tsx - Refactored to match AdvanceRequest pattern
...
import ExpenceClaimCard from './ExpenceClaimCard';
...
const ExpenseClaim = () => {
  ...
  const expenseClaim = useSelector((state: RootState) => state.expenseClaim.expenseClaim);

  const request = useMemo(() => remoteData?.data || expenseClaim, [remoteData, expenseClaim]);
  ...
  const { updateExpenseClaim, isPending: isUpdating } = useUpdateExpenseClaim(requestId!);
  ...
  const { downloadPdf, isGenerating } = usePdfDownload({
    filename: `${infoConfig.abbriviation}-ExpenseClaim-${request?.id}`,
    multiPage: true,
    titleOptions: {
      text: `${infoConfig.abbriviation} Expense Claim : ${capitalizeFirstLetter(request?.status ?? '')}`,
    },
    footerCode: {
      label: `${infoConfig.abbriviation} Expense Claim`,
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    updateExpenseClaim({ data: payload });
  };
    ...
    <div className="flex flex-col space-y-3 pb-80">
      ...
      <div className="sticky -top-8 z-10 bg-[#F8F8F8] pt-4 md:pt-6 pb-3 space-y-1.5 border-b">
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={() => navigate('/expense-claims')}>
