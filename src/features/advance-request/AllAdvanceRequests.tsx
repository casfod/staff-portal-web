...
const AllAdvanceRequests = () => {
  ...
  const handleAction = (request: AdvanceRequestType) => {
    dispatch(setAdvanceRequest(request));
    navigate(`/advance-requests/request/${request.id}`);
  };

  const handleEdit = (request: AdvanceRequestType) => {
    dispatch(setAdvanceRequest(request));
  ...
  return (
    <div className="flex flex-col space-y-3 pb-16">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigate("/advance-requests/create-request")} // Use relative path here
