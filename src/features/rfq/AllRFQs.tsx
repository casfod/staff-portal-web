...
export function AllRFQs() {
  ...
  const rfqs = useMemo(() => data?.data?.rfqs ?? [], [data]);
  ...
  const handleAction = (rfq: RFQType) => {
    dispatch(setRFQ(rfq));
    navigate(`/procurement/rfq/${rfq.id}`);
  };

  const handleEdit = (rfq: RFQType) => {
    dispatch(setRFQ(rfq));
    ...
    <div className="flex flex-col space-y-3 pb-80">
      <div className="sticky top-0 z-10 bg-[#F8F8F8] pt-4 md:pt-6 pb-3 space-y-1.5 border-b">
        ...
        <div className="flex justify-between items-center">
          <TextHeader>Request for Quotations</TextHeader>

          <div className="flex items-center gap-3">
            {(currentUser.role === "SUPER-ADMIN" ||
              currentUser.procurementRole.canCreate) && (
              <Button onClick={() => navigate("/procurement/rfq/create-rfq")}>
      ...
      {/* Pagination */}
      {(rfqs.length >= limit || totalPages > 1) && (
        <Pagination
          currentPage={page}
