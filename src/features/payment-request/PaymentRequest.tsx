// PaymentRequest.tsx - Refactored to match AdvanceRequest pattern
...
import { IComment, IRequestDetailFormData } from '../../interfaces';
...
const PaymentRequest = () => {
  ...
  // Data fetching
  const { data: remoteData, isLoading, isError } = usePaymentRequest(requestId!);
  const paymentRequest = useSelector((state: RootState) => state.paymentRequest.paymentRequest);

  const request = useMemo(() => remoteData?.data || paymentRequest, [remoteData, paymentRequest]);
  ...
  const { updatePaymentRequest, isPending: isUpdating } = useUpdatePaymentRequest(requestId!);
  ...
  const { downloadPdf, isGenerating } = usePdfDownload({
    filename: `${infoConfig.abbriviation}-PaymentRequest-${request?.id}`,
    multiPage: true,
    titleOptions: {
      text: `${infoConfig.abbriviation} Payment Request : ${capitalizeFirstLetter(request?.status ?? '')}`,
    },
    footerCode: {
      label: `${infoConfig.abbriviation} Payment Request`,
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    updatePaymentRequest({ data: payload });
  };
    ...
    <div className="flex flex-col space-y-3 pb-80">
      ...
      <div className="sticky -top-8 z-10 bg-[#F8F8F8] pt-4 md:pt-6 pb-3 space-y-1.5 border-b">
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={() => navigate('/payment-requests')}>
