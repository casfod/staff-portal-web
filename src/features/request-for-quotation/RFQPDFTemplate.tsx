...
const RFQPDFTemplate: React.FC<RFQPDFTemplateProps> = ({
  ...
  const grandTotal = rfqData.itemGroups?.reduce((sum, item) => sum + (item.total || 0), 0);

  // Get the address based on casfodAddressId
  const getAddress = () => {
    const address = casfodTemplateAddress[rfqData.casfodAddressId as keyof typeof casfodTemplateAddress];
    if (!address) {
    ...
    >
      ...
      <div className="flex justify-between items-start mb-8 border-b-2 border-gray-300 pb-4">
        ...
        <div className="text-right">
          <h2 className="text-lg font-bold text-gray-700">Request for Quotation</h2>
          <p className="text-md font-semibold text-blue-600 mt-1">{rfqData.rfqCode}</p>
          <p className="text-sm text-gray-500 mt-1">
            RFQ Date: {rfqData.rfqDate ? formatToDDMMYYYY(rfqData.rfqDate) : 'N/A'}
          </p>
        </div>
      </div>
      ...
      <div className="mb-6">
        <p className="leading-relaxed text-md text-gray-700">
          The {infoConfig.name} ({infoConfig.abbriviation}) invites your company to submit a price
          quotation for the following items in accordance with the requirements detailed below. You
          may use your company format, or fill up the table below. Please read the instruction
          carefully.
        </p>
      </div>
      ...
      <div className="mb-6">
        ...
        <ul className="text-md text-gray-700 space-y-2">
          <li className="flex items-start">
            ...
            <span>Payment terms: Payment will be made after delivery and acceptance of goods</span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>All responses to this RFQ will be treated as confidential.</span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>CASFOD reserves the right to accept or reject any quotations.</span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>Bidders shall adhere to all the requirements of this RFQ</span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>
              CASFOD enforces a withholding tax deduction on the total contractual sum. CASFOD reserves
              the right to validate documentation accuracy. Inquiries or clarifications on tax
              matters should be communicated before the RFQ submission deadline. Participation
              implies vendor agreement to the outlined withholding tax and VAT exemption policies
            </span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>Language in which Quotation will be submitted must be in English.</span>
          </li>
        </ul>
      </div>
      ...
      {/* Evaluation Criteria */}
      <div className="mb-6">
        <h3 className="font-bold mb-3 text-gray-800 border-b pb-1">Evaluation Criteria</h3>
        <ul className=" text-gray-700 space-y-2">
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>
              Bidder can legally operate in Nigeria as a business entity (attach copies of proof of
          ...
          <li className="flex items-start">
            <span className="mr-2">•</span>
            <span>
              Completely filled, signed and stamped Quotation page to page to be submitted
            </span>
          </li>
          <li className="flex items-start">
            <span className="mr-2">•</span>
          ...
          <tbody>
            {rfqData.itemGroups?.map((item, index) => (
              <React.Fragment key={index}>
                <tr className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="border border-gray-300 p-2 text-sm text-right">
                    {item.total && item.total > 0 ? item.total.toLocaleString() : ''}
                  </td>
                </tr>
            ...
            <tr className="bg-gray-100 font-bold">
              ...
              <td className="border border-gray-300 p-2 text-right text-sm">
                ₦{grandTotal && grandTotal > 0 ? grandTotal.toLocaleString() : '0'}
              </td>
            </tr>
          </tbody>
        </table>
          ...
          <tbody>
            <tr>
              <td className="border border-gray-300 p-2 font-semibold bg-gray-100 w-1/3 text-sm">
                Delivery Period (in days, from receipt of ${infoConfig.abbriviation} Purchase Order)
              </td>
            ...
            <tr>
              <td className="border border-gray-300 p-2 font-semibold bg-gray-100 text-sm">
                Bid Validity Period (in days from receipt of ${infoConfig.abbriviation} Purchase Order):
              </td>
              <td className="border border-gray-300 p-2 text-sm"></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td className="border border-gray-300 p-2 text-sm">
                <p className="text-sm text-gray-500 mt-1">
                  {rfqData.deadlineDate ? formatToDDMMYYYY(rfqData.deadlineDate) : 'N/A'}
                </p>
              </td>
            </tr>
          </tbody>
      ...
      <div className="mt-8 pt-4 border-t border-gray-300">
        <div className="grid grid-cols-2 gap-8">
          ...
          <div className="text-center">
            ...
            <p className="text-sm">
              {rfqData.createdBy?.firstName} {rfqData.createdBy?.lastName}
            </p>
            <p className="text-xs text-gray-600">{rfqData.createdBy?.email}</p>
            {renderSignature(rfqData.createdBy!)}
          </div>
          ...
          {/* <div className="text-center">
            ...
            {rfqData.approvedBy ? (
              <>
                <p className="text-sm">
                  {rfqData.approvedBy.firstName} {rfqData.approvedBy.lastName}
                </p>
                <p className="text-xs text-gray-600">{rfqData.approvedBy.email}</p>
                {renderSignature(rfqData.approvedBy)}
              </>
            ) : (
              <div className="border-t border-gray-300 pt-8 mt-2">
