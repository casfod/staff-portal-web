import { IGoodsReceived, IGRNItem } from '../../interfaces';
import { useParams } from 'react-router-dom';
import DetailContainer from '../../components/custom/DetailContainer';
import { FileText, Tag, Package, CheckCircle, XCircle } from 'lucide-react';
...
interface GRNField {
  id: string;
  label: string;
  content: string | number | boolean | ReactElement | IGRNItem[];
  icon?: ReactElement;
  isBlock?: boolean;
  isArray?: boolean;
  isgrnItems?: boolean;
...
export const GRNDetails = ({ grn }: GRNDetailsProps) => {
  ...
  const purchaseOrder = typeof grn.purchaseOrder === 'object' ? grn.purchaseOrder : null;
          ...
          >
            ...
            <div
              className={`flex items-center gap-2 mb-3 pb-2 border-b ${
                section.title === 'Receipt Summary' ? 'border-slate-300' : 'border-gray-300'
              }`}
            >
              <div
                className={section.title === 'Receipt Summary' ? 'text-slate-600' : 'text-gray-600'}
              >
                {section.icon}
              </div>
              <h3
                className={`text-lg font-semibold ${
                  section.title === 'Receipt Summary' ? 'text-slate-800' : 'text-gray-800'
                }`}
              >
                {section.title}
              </h3>
              ...
              {section.fields.map(field => (
                <div
                  ...
                  className={`${field.isBlock ? 'md:col-span-2' : ''} ${
                    section.title === 'Receipt Summary' ? 'flex items-center gap-3' : ''
                  }`}
                >
                  <div
                  ...
                  >
                    {field.icon && (
                      <div
                        className={`mt-0.5 ${
                          section.title === 'Receipt Summary' ? 'text-slate-700' : 'text-gray-500'
                        }`}
                      >
                        {field.icon}
                      </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <label
                        className={`block fon text-sm font-extrabold mb-1 uppercase tracking-wide ${
                          section.title === 'Receipt Summary' ? 'text-slate-600' : 'text-gray-600'
                        }`}
                      >
                        {field.label}
                      </label>
                      ...
                      {field.isgrnItems ? (
                        <div className="space-y-3">
                          {(field.content as IGRNItem[]).map((item, index) => (
                            <div
                              key={index}
                              className={`border rounded-lg p-3 bg-white shadow-sm ${
                                item.isFullyReceived ? 'bg-green-50 border-green-200' : ''
                              }`}
                            >
                              <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
                                <div>
                                  <span className="font-semibold text-gray-600">Item Name:</span>
                                  <p className="mt-1">{getItemName(item.itemId)}</p>
                                </div>
                                <div>
                                  <span className="font-semibold text-gray-600">Ordered:</span>
                                ...
                                <div>
                                  ...
                                  <p
                                    className={`mt-1 ${
                                      item.isFullyReceived ? 'text-green-600 font-bold' : ''
                                    }`}
                                  >
                                    {item.numberReceived}
                                  </p>
                                ...
                                <div>
                                  <span className="font-semibold text-gray-600">Difference:</span>
                                  <p
                                    className={`mt-1 ${
                                      item.difference === 0 ? 'text-green-600' : 'text-yellow-600'
                                    }`}
                                  >
                                    {item.difference}
                                  </p>
                          ...
                          <div className="text-right pt-2 border-t">
                            <div className="grid grid-cols-3 gap-4 text-sm">
                              <div>
                                <span className="font-semibold">Total Ordered: </span>
                                <span>
                                  {grn.grnItems.reduce((sum, item) => sum + item.numberOrdered, 0)}
                                </span>
                              </div>
                              <div>
                                <span className="font-semibold">Total Received: </span>
                                <span className="text-green-600">
                                  {grn.grnItems.reduce((sum, item) => sum + item.numberReceived, 0)}
                                </span>
                              </div>
                              <div>
                                <span className="font-semibold">Total Balance: </span>
                                <span className="text-yellow-600">
                                  {grn.grnItems.reduce((sum, item) => sum + item.difference, 0)}
                                </span>
                              </div>
                            </div>
                          </div>
