// TravelRequest.tsx - Refactored to match AdvanceRequest pattern
...
const TravelRequest = () => {
  ...
  const request = useMemo(() => remoteData?.data || travelRequest, [remoteData, travelRequest]);
  ...
  const { updateTravelRequest, isPending: isUpdating } = useUpdateTravelRequest(requestId!);
  ...
  const { downloadPdf, isGenerating } = usePdfDownload({
    filename: `${infoConfig.abbriviation}-TravelRequest-${request?.id}`,
    multiPage: true,
    titleOptions: {
      text: `${infoConfig.abbriviation}Travel Request : ${capitalizeFirstLetter(request?.status ?? '')}`,
    },
    footerCode: {
      label: `${infoConfig.abbriviation} Travel Request`,
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    updateTravelRequest({ data: payload });
  };
    ...
    <div className="flex flex-col space-y-3 pb-80">
      ...
      <div className="sticky -top-8 z-10 bg-[#F8F8F8] pt-4 md:pt-6 pb-3 space-y-1.5 border-b">
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={() => navigate('/travel-requests')}>
