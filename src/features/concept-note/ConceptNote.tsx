// ConceptNote.tsx - Refactored to match AdvanceRequest pattern
...
const ConceptNote = () => {
  ...
  const request = useMemo(() => remoteData?.data || conceptNote, [remoteData, conceptNote]);
  ...
  const { updateConceptNote, isPending: isUpdating } = useUpdateConceptNote(requestId!);
  ...
  const { downloadPdf, isGenerating } = usePdfDownload({
    filename: `${infoConfig.abbriviation}-ConceptNote-${request?.id}`,
    multiPage: true,
    titleOptions: {
      text: `${infoConfig.abbriviation} Concept Note : ${capitalizeFirstLetter(request?.status ?? '')}`,
    },
    footerCode: {
      label: `${infoConfig.abbriviation} Concept Note`,
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    updateConceptNote({ data: payload });
  };
    ...
    <div className="flex flex-col space-y-3 pb-80">
      ...
      <div className="sticky -top-8 z-10 bg-[#F8F8F8] pt-4 md:pt-6 pb-3 space-y-1.5 border-b">
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={() => navigate('/concept-notes')}>
