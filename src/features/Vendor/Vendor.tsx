...
const Vendor = () => {
  ...
  const { updateVendorStatus, isPending: isUpdatingStatus } = useUpdateVendorStatus();

  // PDF logic
  const pdfContentRef = useRef<HTMLDivElement>(null);
  const { downloadPdf, isGenerating } = usePdfDownload({
    filename: `${infoConfig.abbriviation}-Vendor-${vendor?.vendorCode}`,
    multiPage: true,
    titleOptions: {
      text: `${infoConfig.abbriviation} Vendor Registration Details`,
    },
    footerCode: {
      label: 'CASFOD Vendor',
  ...
  const canApprove = vendor?.status === 'pending' && vendor?.approvedBy?.id === currentUser?.id;

  const tableHeadData = ['Business Name', 'Status', 'Vendor Code', 'Contact Person', 'Actions'];
  ...
  const tableRowData = [
    ...
    {
      id: 'status',
      content: <StatusBadge status={vendor?.status || 'N/A'} />,
    },
                ...
                <tr>
                  <td colSpan={5}>
                    <div className="border border-gray-300 px-3 py-2.5 md:px-6 md:py-3 rounded-md h-auto relative">
                      ...
                      <RequestCommentsAndActions request={vendor!} handleAction={() => {}} />
