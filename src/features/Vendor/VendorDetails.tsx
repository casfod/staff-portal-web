// features/Vendor/VendorDetails.tsx
...
export const VendorDetails = ({ vendor }: VendorDetailsProps) => {
  ...
  const canManageFiles = vendor.createdBy?.id === currentUser?.id && vendor.status !== 'approved';

  return (
    <DetailContainer>
      <div className="flex flex-col gap-6 w-full text-sm md:text-base break-words">
        {vendorSections.map((section, sectionIndex) => (
          <div
            key={sectionIndex}
            className={`rounded-lg p-4 border ${
              section.title === 'Contact Information' || section.title === 'Bank Account Details'
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <label
                        className={`block font-extrabold text-sm mb-1 uppercase tracking-wide ${
                          section.title === 'Contact Information' ||
                          section.title === 'Bank Account Details'
                            ? 'text-slate-600'
                            : 'text-gray-600'
                      ...
                      {field.isArray ? (
                        <div className="flex flex-wrap gap-2">
                          {(field.content as string[]).length > 0 ? (
                            (field.content as string[]).map((category, index) => (
                              <span
                                key={index}
