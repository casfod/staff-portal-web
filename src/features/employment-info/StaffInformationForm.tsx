...
const StaffInformationForm: React.FC<StaffInformationFormProps> = ({
  ...
  const [formData, setFormData] = useState<EmploymentInfoType>(() => {
    ...
    if (staffInfo) {
      if (staffInfo.employmentInfo) {
        // Find the supervisor name from users list if supervisorId exists
        let supervisorName =
          staffInfo.employmentInfo.jobDetails?.supervisor || "";

        // If we have a supervisorId but no supervisor name, try to find the name from users list
        if (
          staffInfo.employmentInfo.jobDetails?.supervisorId &&
          !supervisorName
        ) {
          const supervisor = users.find(
            (u) => u.id === staffInfo.employmentInfo?.jobDetails?.supervisorId
          );
          if (supervisor) {
        ...
        return {
          ...
          jobDetails: {
            ...
            supervisorId:
              staffInfo.employmentInfo.jobDetails?.supervisorId || null,
          },
          emergencyContact: {
  ...
  useEffect(() => {
    ...
    if (data?.data?.employmentInfo) {
      ...
      if (existingData.jobDetails?.supervisorId && !supervisorName) {
        const supervisor = users.find(
          (u) => u.id === existingData.jobDetails?.supervisorId
        );
        if (supervisor) {
          supervisorName = `${supervisor.first_name} ${supervisor.last_name}`;
        }
      ...
      setFormData((prev) => ({
        ...
        jobDetails: {
          ...
          supervisor: supervisorName,
          supervisorId: existingData.jobDetails?.supervisorId || null,
        },
        emergencyContact: {
  ...
  //     // Set both the supervisor name (for display) and the supervisorId
  //     handleFormChange("jobDetails", "supervisor", selectedOption.name);
  //     handleFormChange("jobDetails", "supervisorId", selectedId);
  //   } else {
  //     // Clear both if no selection
  //     handleFormChange("jobDetails", "supervisor", "");
  //     handleFormChange("jobDetails", "supervisorId", null);
  //   }
  // };
  ...
  // Find the currently selected supervisor ID based on the supervisor name or ID
  const getSelectedSupervisorId = () => {
    const currentSupervisorName = formData.jobDetails?.supervisor;
    const currentSupervisorId = formData.jobDetails?.supervisorId;

    // If we have a supervisorId, use that
    ...
    if (currentSupervisorName) {
      const user = users.find(
        (u) => `${u.first_name} ${u.last_name}` === currentSupervisorName
      );
      return user?.id || "";
    }
