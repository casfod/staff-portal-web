...
const CustomSelect: React.FC<SelectProps> = ({
  ...
  const [selectedValue, setSelectedValue] = useState<null | string>(value);
  ...
  useEffect(() => {
    if (isOpen && optionsRef.current && selectedValue) {
      const selectedIndex = filteredOptions.findIndex(o => o.id === selectedValue);
      if (selectedIndex >= 0) {
  ...
  return (
    <div className="relative w-full md:w-[250px]" ref={dropdownRef} onKeyDown={handleKeyDown}>
      {/* Label */}
      {label && (
      ...
      >
        <span className="truncate">{selectedOption ? selectedOption.name : customLabel}</span>
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={`transform transition-transform ${isOpen && !disabled ? 'rotate-180' : ''}`}
          >
            ▼
          </span>
        </div>
      ...
      {isOpen && !disabled && (
        <div
          ...
          style={{
            maxHeight: typeof optionsHeight === 'number' ? `${optionsHeight}px` : optionsHeight,
          }}
          role="listbox"
          ref={optionsRef}
        >
          ...
          {filteredOptions.length > 0 ? (
            filteredOptions.map((option, index) => (
              <div
                key={option.id}
                className={`px-4 py-2 hover:bg-gray-100 truncate cursor-pointer ${
